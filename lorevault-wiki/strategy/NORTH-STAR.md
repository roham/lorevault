...
It is for the collector who pulls a Reichenbach card and loses a morning to the Watson casebook fragment that came folded inside it. It is for the Dracula-Daily reader who, on May 3, 2027, opens their vault and finds 365 letters waiting in canonical-date order. It is for the Sherlockian completist who notices a chessboard pattern in the rocks at the Falls and is never told why — only that the same chessboard appeared three Sets earlier on Moriarty's floor. It is for the gambler who pulls the Apex Ultimate on Day 0 of BS-4 and watches the Council confirm, live on stream, that the smudge on the card is a residue they will not name. We are building it parallel to the v1 site, not iterating, because the v1 site is a Top Shot clone with literary names painted on; the rebirth is a different product underneath, and you cannot refactor your way from one to the other.
...
## 3. The Bar — Seven Specific Yardsticks
...
**Yardstick 5 — Operational.** *One Set per month, sustained, AI-augmented, Canon Council gating.* 20 Moments × 12 months = 240 Moments per Series, ~720 distinct renders with Parallels. R4's pipeline: 8 FTE, ~$0.30 FLUX cost per card, ~15-20 minutes of human attention per card, four Council-gated review points (Brief, Art Direction, Voice, Set-Lock). The Council's bandwidth is the binding constraint — four voting seats (Canon Lead, Voice Lead, Footnoter, Art Director) can defend ~60 Moments/month at Set-Lock without skipping the Mosaic Test. Two Sets per month is the steady state; one Set per month is the floor; zero Sets per month is failure.

**Yardstick 6 — Audience.** *100+ paid pack opens in beta cohort with ≥40% return-rate at week 4, plus a stepped Day-1 email-subscription floor that ramps with cohort maturity.* The Day-30 retention of *Jonathan's Journal* subscribers — the closest analogue to Dracula Daily's organic re-open rate at smaller scale — is the bellwether. ≥40% Week-4 return on the beta cohort proves the cadence holds; below 30% the cadence is wrong; below 20% the product is wrong. Paid pack opens > 100 in beta is a low bar; what matters is the curve, not the level. The Day-1 funnel target ramps explicitly: **15% Day-1 Journal subscription at Series 1 launch month; 25% by Series 1 month 6; 35% by Series 2 launch.** The daemon scores against this moving floor — failing the stepped target is the early-warning that the funnel is not compounding. We are looking for a habit, not a transaction.

**Yardstick 7 — Thrilled.** *Unprompted social emission rate.* The CEO's bar is *thrilled*; Yardsticks 1–6 measure the property and the funnel, but none of them measures whether the audience is *thrilled enough to scream-text the screenshot.* Y7 closes that gap. Measurement: percentage of beta-cohort users who post a card image, a quote screenshot, or a Lampblack-thread theory to public channels (Tumblr / X / Reddit / Discord) without a referral incentive, within their first 30 days of activation. Daemon-side: scrape via Brand24 / Tagboard search for `LoreVault` mentions weekly and count unique-poster IDs; pre-analytics proxy is unique-Twitter-image-shares pulled from `og:image` referers in Vercel logs. **Target: ≥15% of activated users emit at least one quote-screenshot, card image, or Lampblack-theory post within 30 days. Floor: 5%.** Mapped to **Track A (art) + Track B (copy)** — the screen-shot-able moments are made of words and pictures. This is the audience-thrilled metric: a daemon advancing Y1–Y6 to floor while Y7 sits below 5% has built a property that is good but never screen-shot, which is technically a daemon-success and a CEO-failure. Y7 forecloses that outcome.

These seven are the Series 1 gate. Pass all seven and Series 2 commissioning unlocks. Fail any one and Series 1 holds until the gap closes.
...
**Decision One — The Canon Council overrides the founder.** Four voting seats — Chief Worldbuilder (Canon Lead), Voice Lead (four non-Topsy registers, agent-assisted), Footnoter (Topsy-only, hand-written), Art Director — empowered in writing from drop one to override the founder on canon questions. The founder is a non-voting fifth seat who can be overridden by unanimous Seats 1, 2a, 2b, and 3. The Council's only continuous job is gatekeeping the Spines, the 1:2:4 iceberg ratio, and the eight-shell taxonomy. This is the single highest-leverage operational decision in the property: decoupling the IP from the founder is what makes the Lattice survivable past one bad creative season, one regulatory crisis, or one founder departure. The Pokémon Company structure transplanted at one-tenth scale. The four-seat structure (rather than three) exists because the Voice Lead role splits at drop one — the four-Universe register editor cannot also be the sole hand-writer of all Topsy Footnoter prose at the volumes the property requires (R4 §1, §7). Without this, every other constraint is founder-vetoable and therefore non-binding.
