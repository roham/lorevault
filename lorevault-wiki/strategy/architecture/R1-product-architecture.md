...
**4.1 The Lattice map (`/v2/lattice`).** The only diagrammatic cosmology surface. Five Panes as nodes; eight Shell rings concentric around the origin; active tethers drawn as lines. Tapping a Pane routes to `/v2/universe/:slug`. Tapping a Shell ring opens `/v2/vault/shell/:slug` if the user has cards in that Shell, else `/v2/lore/essay/shells` (the iceberg-safe shells primer). Tapping a tether line routes to `/v2/lore/tether/:tetherId`. The map is mobile-first — vertical at 375px, diagonal at tablet, full geometric at desktop. It is always reachable from the global nav as a small icon, and always rendered on home.
